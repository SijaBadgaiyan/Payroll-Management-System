...
void login()
{
    ...
    cin>>UserName;      //Input username and password
    gotoXY(20,12);
    cout<<"Enter Password : ";
    while(1)
    {
    	...
    	{
			cout<<"*";  //Printing password in the form of string of astrickes
			Password[i]=ch;
			i++;
    	}
	}
	Password[i] = '\0';
    if(strcmp(UserName,"admin")==0 && strcmp(Password,"password")==0) // Only these credentials will be able to access the data
    {
    	system("cls"); //clear system screen
    	loginFrame1();  // user interface component
    	gotoXY(27,10);
    	cout<<"Login Success!!!";
    	gotoXY(21,12);
	...
	else
	{
    	...
    	loginFrame1();
    	gotoXY(27,10);
		printf("Access Denied!!!\a"); //Output if credentials do not match
    	gotoXY(21,12);
    	cout<<"Will be redirected in 3 Seconds...";
    		gotoXY(56,12);
    	//Cdelay(1000);
...
void insert()
{
	...
	TAX =  0.04 * sal;        //Calculation of allowances
	DA = 1.20 * sal;
	PF = 0.12 * sal;
	HRA = sal * 0.27;
	MealA = 300;
...
void  list()   //Displaying data in the file
{
	//system("cls");
	borderNoDelay();
	gotoXY(20,4);
...
void edit()  // Driver code for edit menu
{
	//system("cls");
	borderNoDelay();
	int jobcode;
